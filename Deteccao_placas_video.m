...
cam = Movie('dataset/video_placa3.mp4');
time = cam.totalDuration;
frames = cam.nframes;
step = time/frames;
good = 0;
good1 = 0;
...
while(good == 0)
    ...
    vcort = floor(size(placa2,1)*0.13846);
    ...
    amin1 = size(mask1,1)*size(mask1,2)*0.00238;
    ...
    for i=1:size(vetlab1,2)
        if(vetor2(vetlab1(i)) == 1)
            blob_cidade{i} = p_cidade([v1(vetlab1(1)): v1(vetlab1(i)+s1)], [u1(vetlab1(i)): u1(vetlab1(i)+s1)]);
